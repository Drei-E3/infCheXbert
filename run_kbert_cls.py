...
def add_knowledge_worker(params):
    ...
    for line_id, line in enumerate(sentences):
        ...
        try:
            ...
            elif len([x if 'label' else None in x for x in columns.keys()]) >1 : # multiple classification
                label = [line[columns[x]] if 'label' in x else None for x in columns.keys()]
                text = [line[columns[x]]if (('impression' in x) or ('finding' in x) )else None for x in columns.keys()]
...
def main():
    ...
    def evaluate(args, is_test, metrics='Acc',label_id = -1):
        ...
        for counter, label in enumerate(label_columns):
            labels_ids.append(torch.LongTensor([int(example[1][counter]) for example in dataset]))
        ...
        if not args.mean_reciprocal_rank:
            for i, (input_ids_batch, labels_ids_batch,  mask_ids_batch, pos_ids_batch, vms_batch) in enumerate(batch_loader(batch_size, input_ids, labels_ids, mask_ids, pos_ids, vms)):

                # vms_batch = vms_batch.long()
                vms_batch = torch.LongTensor(vms_batch)

                input_ids_batch = input_ids_batch.to(device)
                label_ids_batch = labels_ids_batch[counter]
            ...
            if metrics == 'Acc':
                ...
                for i, (input_ids_batch, label_ids_batch,  mask_ids_batch, pos_ids_batch, vms_batch) in enumerate(batch_loader(batch_size, input_ids, labels_ids, mask_ids, pos_ids, vms)):
